...
data = pd.read_csv('/content/output.csv',encoding = 'utf-8')
hannanum = Hannanum()
t_noun, s_noun = [],[]
t_nouns, t_count, s_nouns, s_count = [],[],[],[]
Summary = []
Title = []
date = []
SubS = []
...
for i,row in data.iterrows():
  Summary.append(re.sub('<br/>','',str(row['Summary'])))
  SubS.append(re.sub('<br/>|"|,|…|”|“|·',' ',str(row['Summary'])))
...
df = pd.DataFrame({'date':data['Date'],'date_value':date, 'title' : data['Title'],'summary':Summary,'t_element': t_nouns,'t_count':t_count,'s_element': s_nouns, 's_count' : s_count})
